...
//! TDD guard-rails for OKX HTTP candlestick queries.
//!
//! Each test encodes an invariant taken directly from the v5 REST spec:
//!   * omit empty after/before
//!   * cursors must be milliseconds (≤ 13 digits)
//!   * candles endpoint may not exceed 300 rows, history-candles 100 rows
//!   * API forbids sending *both* after **and** before
//!
//! Following the standard Rust exchange crate pattern with embedded validation.
